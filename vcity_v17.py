...
def create_v17_nodes():
    group_name = "VoronoiCity_V17"
    ...
    # 2. Distortion (Skipping detailed noise controls for brevity, using defaults + Seed)
    ...
    n_scale.inputs[3].default_value = 5.0 # Distortion Amount Fixed or Param
    ...
    # --- CAPTURE RANDOM MATERIAL INDEX HERE (Before Split) ---
    # We want 0, 1, 2, 3 as indices.
    n_rand_idx = nodes.new('FunctionNodeRandomValue')
    n_rand_idx.data_type = 'INT'
    n_rand_idx.location = (-1600, 400)
    n_rand_idx.inputs['Min'].default_value = 1 # Start at 1 (0 is Road)
    n_rand_idx.inputs['Max'].default_value = 4 # Up to 4 (Exclusive? Random Int usually Is Max Inclusive or Exclusive? It depends on version. Let's say 4 to be safe)
    # Actually Random Int is [Min, Max] inclusive in older, [Min, Max) in newer? The tooltip usually says.
    # We will aim for indices 1, 2, 3, 4.
    
    n_idx_node = nodes.new('GeometryNodeInputIndex')
    link_safe(n_idx_node.outputs[0], n_rand_idx, "ID") # Use Index of original face
    link_safe(get_parameter("Color Seed"), n_rand_idx, "Seed")
    
    n_capture = nodes.new('GeometryNodeCaptureAttribute')
    n_capture.location = (-1400, 400)
    n_capture.data_type = 'INT'
    n_capture.domain = 'FACE'
    
    link_safe(n_dual.outputs[0], n_capture, "Geometry")
    link_safe(n_rand_idx.outputs["Value"], n_capture, "Value")
    
    # ROADS
    n_mat_road = nodes.new('GeometryNodeSetMaterialIndex') # Use Index!
    n_mat_road.location = (-800, -300)
    link_safe(n_capture.outputs["Geometry"], n_mat_road, "Geometry") # Use captured geo
    n_mat_road.inputs["Material Index"].default_value = 0 # Road is Slot 0
    
    # BUILDINGS
    n_split = nodes.new('GeometryNodeSplitEdges')
    n_split.location = (-1200, 200)
    link_safe(n_capture.outputs["Geometry"], n_split, "Mesh")
    ...
    # Use the CAPTURED attribute as ID for height too? 
    # Or just capture another random value. 
    # Let's use Index logic again, or Position.
    # Since we split edges, Index changed. 
    # We can use the Captured Random Index as part of the seed for height? No, height should be independent.
    # Let's just use Position for height as before, stable.
    ...
    # ASSIGN MATERIAL INDEX (Calculated earlier)
    n_set_idx = nodes.new('GeometryNodeSetMaterialIndex')
    n_set_idx.location = (-600, 200)
    link_safe(n_ext.outputs[0], n_set_idx, "Geometry")
    link_safe(n_capture.outputs["Attribute"], n_set_idx, "Material Index") # The captured integer!
...
def setup_scene_v17():
    print("=" * 40)
    print("Creating V17 (Material Slot Randomness)...")
    ...
    # Create Object
    mesh = bpy.data.meshes.new("CityV17Mesh")
    obj = bpy.data.objects.new("CityV17", mesh)
    bpy.context.scene.collection.objects.link(obj)
    
    # Create Palette of Materials
    colors = [
        (0.05, 0.05, 0.05, 1), # 0: Road (Dark)
        (0.8, 0.2, 0.2, 1),    # 1: Red Building
        (0.2, 0.2, 0.8, 1),    # 2: Blue Building
        (0.8, 0.5, 0.0, 1),    # 3: Orange Building
        (0.2, 0.8, 0.2, 1),    # 4: Green Building
    ]
    ...
    mod = obj.modifiers.new("CityGenV17", 'NODES')
    mod.node_group = ng
    
    print("V17 Success. Distinct Material Slots used.")
