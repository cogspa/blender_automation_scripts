...
def create_v13_nodes():
    group_name = "VoronoiCity_V13"
    ...
    # We want a random value per FACE (or Mesh Island).
    # Since we split edges, faces are unique.
    ...
    # Use different seed for color so it doesn't match height exactly
    n_seed_math = nodes.new('ShaderNodeMath')
    n_seed_math.operation = 'ADD'
    n_seed_math.inputs[1].default_value = 55.0
    link_safe(get_parameter("Seed"), n_seed_math, 0)
    
    link_safe(n_idx.outputs[0], n_rand_col, "ID")
    link_safe(n_seed_math.outputs[0], n_rand_col, "Seed")
    
    n_store_col = nodes.new('GeometryNodeStoreNamedAttribute')
    n_store_col.location = (-800, 500)
    n_store_col.data_type = 'FLOAT'
    n_store_col.domain = 'FACE' # Color per face (building)
    n_store_col.name = "RandomColor"
    
    link_safe(n_ext.outputs[0], n_store_col, "Geometry")
    link_safe(n_rand_col.outputs[1], n_store_col, "Value") # Value is [1] for Float random? No, [0] is Value, [1] is 'Value'? Wait.
    # FunctionNodeRandomValue Float output is index 1 ("Value")
...
def setup_scene_v13():
    print("=" * 40)
    print("Creating V13 (Random Colors per Building)...")
    ...
    mesh = bpy.data.meshes.new("CityV13Mesh")
    obj = bpy.data.objects.new("CityV13", mesh)
    bpy.context.scene.collection.objects.link(obj)
    
    m_road = bpy.data.materials.new("RoadMatV13")
    m_road.use_nodes = True
    m_road.node_tree.nodes['Principled BSDF'].inputs[0].default_value = (0.05, 0.05, 0.05, 1)

    # --- ADVANCED MATERIAL WITH ATTRIBUTE ---
    m_bldg = bpy.data.materials.new("BldgMatV13")
    ...
    attr_node.attribute_name = "RandomColor"
    attr_node.location = (-500, 200)
    
    # Color Ramp
    ramp = nodes.new('ShaderNodeValToRGB')
    ramp.location = (-300, 200)
    # Customize Ramp (Blue to Orange to White)
    ramp.color_ramp.elements[0].color = (0.0, 0.1, 0.4, 1) # Dark Blue
    ramp.color_ramp.elements[1].color = (1.0, 0.5, 0.0, 1) # Orange
    # Add middle element
    ramp.color_ramp.elements.new(0.5)
    ramp.color_ramp.elements[1].color = (0.1, 0.6, 0.8, 1) # Cyan
    ...
    mod = obj.modifiers.new("CityGenV13", 'NODES')
    mod.node_group = ng
