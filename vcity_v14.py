...
def create_v14_nodes():
    group_name = "VoronoiCity_V14"
    ...
    link_safe(get_parameter("Color Seed"), n_rand_col, "Seed") # New Color Slider
...
def setup_scene_v14():
    print("=" * 40)
    print("Creating V14 (Color Seed Slider)...")
    ...
    mesh = bpy.data.meshes.new("CityV14Mesh")
    obj = bpy.data.objects.new("CityV14", mesh)
    bpy.context.scene.collection.objects.link(obj)
    
    m_road = bpy.data.materials.new("RoadMatV14")
    m_road.use_nodes = True
    m_road.node_tree.nodes['Principled BSDF'].inputs[0].default_value = (0.05, 0.05, 0.05, 1)

    m_bldg = bpy.data.materials.new("BldgMatV14")
    ...
    mod = obj.modifiers.new("CityGenV14", 'NODES')
    mod.node_group = ng
