...
def create_v21_nodes():
    group_name = "VoronoiCity_V21"
    ...
    n_id_noise.noise_dimensions = '4D' 
    n_id_noise.inputs['Scale'].default_value = 100.0 # High freq noise
...
def setup_scene_v21():
    print("=" * 40)
    print("Creating V21 (Force Viewport Material Color)...")
    ...
    mesh = bpy.data.meshes.new("CityV21Mesh")
    obj = bpy.data.objects.new("CityV21", mesh)
    ...
    mod = obj.modifiers.new("CityGenV21", 'NODES')
    mod.node_group = ng
    
    # Fix Material bindings
    ...
    # --- FORCE VIEWPORT SHADING ---
    # Try getting the current area
    for area in bpy.context.screen.areas:
        if area.type == 'VIEW_3D':
            for space in area.spaces:
                if space.type == 'VIEW_3D':
                    # Force Solid + Material Color
