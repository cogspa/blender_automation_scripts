...
def create_v7_1_nodes():
    group_name = "VoronoiCity_V7_1"
    ...
    link_safe(n_flat.outputs[0], n_set_pos, 2)
    ...
    # 4. SPLIT EDGES (Crucial for individual faces)
    ...
    # 6. Extrude (Random Height using Index)
    ...
    n_rand.location = (-800, -300)
    n_rand.data_type = 'FLOAT'
    
    # Input ID = INDEX (The simplest way)
    # Since we split edges, every face is a generic index sequence? 
    # Actually, Dual Mesh faces are stable.
    n_idx = nodes.new('GeometryNodeInputIndex')
    n_idx.location = (-1000, -300)
    
    link_safe(n_in.outputs[4], n_rand, 0) # Min
    link_safe(n_in.outputs[5], n_rand, 1) # Max
    link_safe(n_idx.outputs[0], n_rand, 2) # ID input
...
def create_scene_v7_1():
    print("Creating Voronoi City V7.1 (Simplified Logic)...")
    ...
    m_bldg.node_tree.nodes['Principled BSDF'].inputs[0].default_value = (0.8, 0.4, 0.1, 1) # Orange
    obj.data.materials.append(m_bldg)
    
    bpy.context.view_layer.objects.active = obj
    
    try:
        ng = create_v7_1_nodes()
