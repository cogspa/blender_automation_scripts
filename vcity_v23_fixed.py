...
def create_v23_nodes():
    ...
    # Use Convex Hull for simple wire network
    ...
    n_sag_scale.inputs[1].default_value = -0.5
...
def create_material_safe(name, base_color, metallic=0.0, roughness=0.5, emission_color=None, emission_strength=0.0):
    """Create a material with Blender 4.0+ compatibility"""
    mat = bpy.data.materials.new(name)
    mat.use_nodes = True
    bsdf = mat.node_tree.nodes.get('Principled BSDF')
    
    if bsdf:
        # Base Color - input 0 in all versions
        bsdf.inputs[0].default_value = base_color
        
        # Metallic - try by name first, then by index
        if 'Metallic' in bsdf.inputs:
            bsdf.inputs['Metallic'].default_value = metallic
        elif len(bsdf.inputs) > 1:
            bsdf.inputs[1].default_value = metallic
            
        # Roughness - try by name first, then by index
        if 'Roughness' in bsdf.inputs:
            bsdf.inputs['Roughness'].default_value = roughness
        elif len(bsdf.inputs) > 2:
            bsdf.inputs[2].default_value = roughness
        
        # Emission - Blender 4.0+ uses "Emission Color" and "Emission Strength"
        # Earlier versions used just "Emission"
        if emission_color and emission_strength > 0:
            # Try Blender 4.0+ names first
            if 'Emission Color' in bsdf.inputs:
                bsdf.inputs['Emission Color'].default_value = emission_color
                if 'Emission Strength' in bsdf.inputs:
                    bsdf.inputs['Emission Strength'].default_value = emission_strength
            # Fall back to older Blender "Emission" input
            elif 'Emission' in bsdf.inputs:
                # In older versions, emission color was multiplied by strength internally
                bsdf.inputs['Emission'].default_value = emission_color
                # Try to set emission strength if it exists
                if 'Emission Strength' in bsdf.inputs:
                    bsdf.inputs['Emission Strength'].default_value = emission_strength
...
def setup_scene_v23():
    print("=" * 40)
    print("Creating V23 (Full City with Windows, Doors, Antennas, Wires)...")
    ...
    # Create materials using the safe function
    mats = {}
    
    # Road - dark gray
    mats["Road"] = create_material_safe(
        "Mat_Road", 
        (0.05, 0.05, 0.05, 1),
    ...
    # Building colors
    mats["Building_Red"] = create_material_safe(
        "Mat_Building_Red",
        (0.7, 0.15, 0.15, 1),
        metallic=0.0, roughness=0.6
    )
    
    mats["Building_Blue"] = create_material_safe(
        "Mat_Building_Blue",
        (0.15, 0.15, 0.7, 1),
        metallic=0.0, roughness=0.6
    )
    
    mats["Building_Orange"] = create_material_safe(
        "Mat_Building_Orange",
        (0.9, 0.45, 0.1, 1),
        metallic=0.0, roughness=0.6
    )
    
    mats["Building_Green"] = create_material_safe(
        "Mat_Building_Green",
        (0.15, 0.6, 0.15, 1),
        metallic=0.0, roughness=0.6
    )
    
    # Window - reflective light blue (like glass)
    mats["Window"] = create_material_safe(
        "Mat_Window",
        (0.6, 0.8, 1.0, 1),
        metallic=0.9, roughness=0.1,
        emission_color=(0.4, 0.6, 0.8, 1),
        emission_strength=0.3
    )
    
    # Door - brown wood
    mats["Door"] = create_material_safe(
        "Mat_Door",
        (0.35, 0.2, 0.1, 1),
        metallic=0.0, roughness=0.7
    )
    
    # Antenna - dark gray metal
    mats["Antenna"] = create_material_safe(
        "Mat_Antenna",
        (0.3, 0.3, 0.3, 1),
    ...
    # Wire - black
    mats["Wire"] = create_material_safe(
        "Mat_Wire",
        (0.1, 0.1, 0.1, 1),
    ...
    # Order: Road, Red, Blue, Orange, Green, Window, Door, Antenna, Wire
    mat_order = ["Road", "Building_Red", "Building_Blue", "Building_Orange", "Building_Green", 
                 "Window", "Door", "Antenna", "Wire"]
    
    for i, node in enumerate(set_mat_nodes):
        if i < len(mat_order):
            node.inputs[2].default_value = mats[mat_order[i]]
    
    print("V23 Success!")
    print("  Features:")
    print("  - Tapered buildings with floor loops")
    print("  - Windows on side faces")
    print("  - Doors at ground level")
    print("  - Antennas on rooftops")
    print("  - Wires connecting buildings")
        ...
        if area.type == 'VIEW_3D':
            for space in area.spaces:
                if space.type == 'VIEW_3D':
                    ...
                    print("Viewport set to SOLID - MATERIAL")
