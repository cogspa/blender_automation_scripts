...
def create_v12_nodes():
    group_name = "VoronoiCity_V12"
    ...
    add_socket("Seed", 'NodeSocketInt', 500)
    ...
    # --- BRANCH A: ROADS (The Base) ---
    # We will simply pass this through, maybe color it
    n_mat_road = nodes.new('GeometryNodeSetMaterial')
    n_mat_road.location = (-800, -300)
    link_safe(n_dual.outputs[0], n_mat_road, "Geometry")
    
    # Capture Attribute "IsBuilding" = 0
    n_store_road = nodes.new('GeometryNodeStoreNamedAttribute')
    n_store_road.location = (-600, -300)
    n_store_road.data_type = 'BOOLEAN'
    n_store_road.name = "IsBuilding"
    n_store_road.inputs['Value'].default_value = False
    link_safe(n_mat_road.outputs[0], n_store_road, 0)

    # --- BRANCH B: BUILDINGS (Individual Islands) ---
    # 1. Split Edges (Ensures physical separation)
    n_split = nodes.new('GeometryNodeSplitEdges')
    n_split.location = (-1400, 200)
    link_safe(n_dual.outputs[0], n_split, "Mesh")
    
    # 2. Inset (Scale Elements)
    ...
    link_safe(n_idx.outputs[0], n_rand, "ID")
    ...
    n_mat_bldg.location = (-800, 200)
    link_safe(n_ext.outputs[0], n_mat_bldg, "Geometry")

    # 5. Lift Buildings slightly (Avoid Z-fighting with road)
    n_lift = nodes.new('GeometryNodeTransform')
    n_lift.location = (-600, 200)
    n_lift.inputs['Translation'].default_value = (0, 0, 0.005)
    link_safe(n_mat_bldg.outputs[0], n_lift, "Geometry")
    
    # 6. Capture Attribute "IsBuilding" = 1
    n_store_bldg = nodes.new('GeometryNodeStoreNamedAttribute')
    n_store_bldg.location = (-400, 200)
    n_store_bldg.data_type = 'BOOLEAN'
    n_store_bldg.name = "IsBuilding"
    n_store_bldg.inputs['Value'].default_value = True
    link_safe(n_lift.outputs[0], n_store_bldg, 0)
    
    # --- JOIN ---
    n_join = nodes.new('GeometryNodeJoinGeometry')
    n_join.location = (0, 0)
    link_safe(n_store_road.outputs[0], n_join, "Geometry")
    link_safe(n_store_bldg.outputs[0], n_join, "Geometry")
...
def setup_scene_v12():
    print("=" * 40)
    print("Creating V12 (Active Sliders + Separation + Tags)...")
    ...
    mesh = bpy.data.meshes.new("CityV12Mesh")
    obj = bpy.data.objects.new("CityV12", mesh)
    bpy.context.scene.collection.objects.link(obj)
    
    # Distinct Colors
    m_road = bpy.data.materials.new("RoadMatV12")
    m_road.use_nodes = True
    m_road.node_tree.nodes['Principled BSDF'].inputs[0].default_value = (0.05, 0.05, 0.05, 1) # Dark Gray

    m_bldg = bpy.data.materials.new("BldgMatV12")
    m_bldg.use_nodes = True
    m_bldg.node_tree.nodes['Principled BSDF'].inputs[0].default_value = (0.7, 0.5, 0.0, 1) # Gold/Orange
    
    obj.data.materials.append(m_road)
    obj.data.materials.append(m_bldg)
    
    bpy.context.view_layer.objects.active = obj
    mod = obj.modifiers.new("CityGenV12", 'NODES')
    mod.node_group = ng
    
    # Assign Materials
    for n in ng.nodes:
        if n.type == 'SET_MATERIAL':
            # Check hierarchy for material assignment logic
            # Buildings are lifted, Roads are flat
            # Or assume Y location
            if n.location[1] > 0: 
